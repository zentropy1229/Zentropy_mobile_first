  ...
  <div class="relative">
    ...
    <form @submit.prevent="send" class="relative">
      <input
        ...
        class="span-text h-2.5 w-full rounded-sm bg-gray-600 px-0.5 outline-none"
      />
    ...
    <!-- 搜尋框框 -->
    <div
      class="absolute z-10 w-full overflow-hidden rounded-sm border-2 border-gray-300 bg-gray-600"
      v-show="showPreviewKeyWord && getStockName.length"
    >
      <ul class="span-text-sm">
        <li
          class="search-items flex cursor-pointer items-center justify-between p-0.5 hover:bg-gray-500"
          v-for="stockDetail of getStockName"
          :key="stockDetail.stock"
          @click.stop.prevent="
            router.push({ name: 'home', query: { stock: stockDetail.stock } })
          "
        >
