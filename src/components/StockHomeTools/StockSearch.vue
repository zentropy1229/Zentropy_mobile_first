  ...
  <div class="relative">
    ...
    <form @submit.prevent="send" class="relative">
      ...
      <button class="y-center absolute right-0.5" @click.prevent="goStockPage">
        <svg class="h-[0.2rem] w-[0.2rem] text-gray-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor">
          <path
      ...
      <!-- delete button -->
      <button class="y-center ounded-full absolute right-3 z-10" v-if="inputStockName" @click="inputStockName = ''">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-[.15rem] w-[.15rem] text-gray-400 hover:text-gray-500" fill="currentColor" viewBox="0 0 16 16">
          <path
    ...
    <!-- 搜尋框框 -->
    <div class="absolute z-10 w-full overflow-hidden rounded-sm border-2 border-gray-300 bg-gray-600" v-show="showPreviewKeyWord && getStockName.length">
      <ul class="span-text">
        <li
          class="search-items flex cursor-pointer items-center justify-between border-b border-gray-400 px-0.5 py-1 hover:bg-gray-700"
        ...
        >
          ...
          <span class="search-items-icons hidden">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-[.15rem] w-[.15rem] text-gray-400" fill="currentColor" viewBox="0 0 16 16">
...
const searchMark = computed(() => {
  return function (stockList) {
    ...
    return `${stockFull.substring(0, index)}<span class="text-orange-400 font-bold">${stockFull.substring(index, index + inputStockName.value.length)}</span>${stockFull.substring(
