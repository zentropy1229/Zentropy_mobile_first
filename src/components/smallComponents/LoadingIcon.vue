...
import { defineProps, onMounted, onUnmounted, toRef } from 'vue'
let errorCount
...
const setErrorCounting = () => {
  if (isLoading.value) {
    let count = 0
    errorCount = setInterval(() => {
      if (count === 8) {
        clearInterval(errorCount)
        return
      }
      count++
      store.commit('setErrorCounting', count)
    }, 1000)
  } else {
    clearInterval(errorCount)
    store.commit('setErrorCounting', 0)
  }
}
onMounted(() => {
  setErrorCounting()
