<template>
  <div class="flex flex-col">
    <span class="mb-0.5 w-5 text-gray-200">{{ userInfo.text }}</span>
    <div class="flex flex-wrap items-center rounded-sm bg-gray-900 p-0.5">
      <div class="subtitle-text-sm py-[0.1rem] text-gray-400" v-if="isEditing">
      ...
      <button
        ...
        @click.prevent="startEditing(userInfo.value)"
        v-else
      >
      ...
      <div class="ml-auto p-0.5" v-if="isEditing">
        <button @click.prevent="updateUserInfo(userInfo.key)">
          <svg
        ...
        <button class="ml-0.5 text-rose-500" @click.prevent="isEditing = false">
    ...
    <span class="ml-0.5 text-rose-500" v-if="error">{{ error }}</span>
  </div>
</template>
