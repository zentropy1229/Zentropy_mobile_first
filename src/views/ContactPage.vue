<template>
  <table class="flex-center bord span-text mt-4">
    <tbody>
      <div class="flex">
    ...
    <table>
      <tbody>
        <tr v-show="result">
          ...
          <th class="pb-0.5">股票市場別</th>
          <th class="pb-0.5">有價證券別</th>
          <th class="pb-0.5">市場別</th>
          <th class="pb-0.5">上市時間</th>
        </tr>
        <tr
        ...
        >
          <td
            v-for="item in Object.entries(list).slice(0, 6)"
            :key="item"
            class="px-1.5 py-0.5"
          >
            {{ item[1] }}
...
const filter = () => {
  if (!checkedList.value.length) {
    ...
    axios
      ...
      .then((res) => {
        console.log(checkedList.value.join(','))
