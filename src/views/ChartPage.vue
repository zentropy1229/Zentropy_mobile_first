...
export default {
  ...
  methods: {
    // 设置折线图
    getLineChart () {
      this.lineChart = markRaw(echarts.init(document.getElementById('lineChart')))
    ...
    calculateMA (dayCount) {
      const result = []
      for (let i = 0, len = this.candleData.values.length; i < len; i++) {
        if (i < dayCount) {
          result.push('-')
          continue
        }
